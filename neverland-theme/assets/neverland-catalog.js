...
(function () {
  ...
  function updateRoomView() {
    ...
    if (dims && img) {
      ...
      // Match the product-page mockup scaling exactly (same mockup-wall.jpg):
      // average wall ≈ 250cm desktop / 350cm mobile; wall spans 65% of scene height.
      ...
      // Clamp to reasonable limits relative to the scene.
  ...
  function initAccordion() {
    // Delegated off document (bound once) so it keeps working even though the
    // accordion markup is injected after the page loads. The previous
    // clone-and-rebind version could silently stop firing.
    if (window.__ppAccordionBound) return;
    window.__ppAccordionBound = true;
    document.addEventListener('click', function (e) {
      const trigger = e.target.closest('.pp-accordion__trigger');
      if (!trigger) return;
      const content = trigger.nextElementSibling;
      const isOpen = trigger.classList.contains('is-open');
      // Single-open accordion: close siblings first.
      const group = trigger.closest('.pp-accordion');
      if (group) {
        group.querySelectorAll('.pp-accordion__trigger').forEach(t => {
