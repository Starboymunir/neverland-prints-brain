...
${items}`;
  ...
  process.on("message", async ({ ids }) => {
    ...
    // Pending DB updates buffer — written by lanes, drained by dedicated writer
    const pendingUpdates = [];
    let lanesFinished = false;
    ...
    // Dedicated DB writers — run in parallel with lanes, never block API calls
    const DB_WRITERS = 2;  // 2 writers per worker (8 workers × 2 = 16 max DB connections)

    async function dbWriter() {
      while (!lanesFinished || pendingUpdates.length > 0) {
        if (pendingUpdates.length === 0) {
          await sleep(50);  // idle pause
          continue;
        }
        const item = pendingUpdates.shift();
        if (!item) continue;
        const ok = await writeOne(item.id, item.result);
        if (ok) tagged++; else errors++;

        // Report progress during writes
    ...
    // Each "lane" is an independent async loop — only does API calls!
    async function lane() {
      while (true) {
        const bi = batchIdx++;
    ...
    // Launch lanes (API) + writers (DB) in parallel — fully decoupled!
    const lanePromises = [];
    for (let i = 0; i < LANES; i++) {
      lanePromises.push(lane());
    }
    const writerPromises = [];
    for (let i = 0; i < DB_WRITERS; i++) {
      writerPromises.push(dbWriter());
    }

    await Promise.all(lanePromises);
    lanesFinished = true;  // signal writers to drain remaining items and stop
  ...
  const NUM_WORKERS = parseInt(getArg("workers") || "8", 10);
